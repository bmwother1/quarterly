...
/** The second line: method, session, and when the work is due. */
function Meta({
  block, isPast, sayOpen = true, tz, due, onShowDeadline, className = '',
}: Pick<BlockProps, 'block' | 'isPast' | 'tz' | 'due' | 'onShowDeadline'> & { sayOpen?: boolean; className?: string }) {
  const parts = metaParts(block, isPast, sayOpen);
  const dueText = due ? `due ${fmtDay(due.at, tz)}${due.allDay ? '' : ` ${fmtTime(due.at, tz)}`}` : null;
  if (parts.length === 0 && !dueText) return null;
        ...
        <>
          ...
          {onShowDeadline ? (
            <button
              ...
              className="underline decoration-[var(--border-strong)] decoration-dashed underline-offset-4 hover:text-[var(--ink)]"
            >
              {dueText}
            </button>
          ) : dueText}
...
export function BlockCard({
  ...
  className = '', style, due, onShowDeadline,
}: BlockProps & {
  className?: string;
  ...
  return (
    ...
    >
      ...
      <Meta
        block={block} isPast={isPast} sayOpen={!label} tz={tz}
...
export function BlockRow({
  block, tz, colour, onComplete, onDrop, isPast, stagger, due, onShowDeadline,
}: BlockProps & {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Meta block={block} isPast={isPast} tz={tz} due={due} onShowDeadline={onShowDeadline} />
