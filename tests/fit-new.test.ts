...
describe('fitting new Canvas work into a planned week', () => {
  ...
  test('the daily ceiling holds across old and new blocks together', () => {
    // The trap: blocks remove hours, so if the per-day allowance were computed
    // from availability alone a full day would take its whole allowance again.
    // planWeek charges what is already on a day against its ceiling.
    const cap = 120;
